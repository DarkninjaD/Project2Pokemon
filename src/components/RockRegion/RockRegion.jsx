...
const RockRegion = () => {
  ...
  //Pokemon are pulled from Rock pokemon
  const genRandomPokemonArray = (rockPokemon) => {
    const randNum = randomNumber();
    const randomPokemonArray = [];
    // get random item
    for (var i = 0; i <= randNum; i++) {
      const randomIndex = Math.floor(Math.random() * rockPokemon.length);
      randomPokemonArray.push(rockPokemon[randomIndex]);
    }
    return randomPokemonArray;
  };

  const randPokemonArray = genRandomPokemonArray(sortedPokemon.rock); //
  // [50] => [5]
  // [23] <= [5]
  ...
  return (
    <div className="body bg-rock" data-testid="background-rock">
      <div className="row">
        ...
        <div className="text-center">
          <PokeDexButton className="pokedex-button" locationName="rock" />
        </div>
      </div>
