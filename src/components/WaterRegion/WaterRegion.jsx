...
const WaterRegion = () => {
  ...
  //Pokemon are pulled from water pokemon
  const genRandomPokemonArray = (waterPokemon) => {
    const randNum = randomNumber();
    const randomPokemonArray = [];
    // get random item
    for (var i = 0; i <= randNum; i++) {
      const randomIndex = Math.floor(Math.random() * waterPokemon.length);
      randomPokemonArray.push(waterPokemon[randomIndex]);
    }
    return randomPokemonArray;
  };

  const randPokemonArray = genRandomPokemonArray(sortedPokemon.water); //
  // [50] => [5]
  // [23] <= [5]
  ...
  return (
    <div className="body bg-water" data-testid="background-water">
      <div className="row">
