...
const GymRegion = () => {
  ...
  //Pokemon are pulled from gym pokemon
  const genRandomPokemonArray = (gymPokemon) => {
    const randNum = randomNumber();
    const randomPokemonArray = [];
    // get random item
    for (var i = 0; i <= randNum; i++) {
      const randomIndex = Math.floor(Math.random() * gymPokemon.length);
      randomPokemonArray.push(gymPokemon[randomIndex]);
    }
    return randomPokemonArray;
  };

  const randPokemonArray = genRandomPokemonArray(sortedPokemon.gym); //
  // [50] => [5]
  // [23] <= [5]
  ...
  return (
    <div className="body bg-gym" data-testid="background-gym">
      <div className="row">
